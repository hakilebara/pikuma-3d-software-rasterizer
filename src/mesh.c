...
void load_obj_file_data(char* filename) {
  ...
  fp = fopen(filename, "r");
  if (fp == NULL) {
    err(EXIT_FAILURE, "Could not open asset file");
  }

  while((getline(&line, &len, fp)) != -1) {
    if (line[0] == 'v' && line[1] == ' ') {
      vec3_t vertex;
      int n = sscanf(line, "v %f %f %f", &vertex.x, &vertex.y, &vertex.z);
      if (n != 3) {
        err(EXIT_FAILURE, "Error parsing .obj file vertices");
      }
    ...
    if (line[0] == 'f' && line[1] == ' ') {
      ...
      if (n != 9) {
        err(EXIT_FAILURE, "Error parsing .obj file faces");
      }
